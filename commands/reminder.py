...
class UniversalAddModal(ui.Modal, title="予定の登録"):
    date_input = ui.TextInput(label="日付 (YYYY-MM-DD)", placeholder="2024-12-25", min_length=10, max_length=10)
    title_input = ui.TextInput(label="タイトル", placeholder="内容を入力", required=True)
    ...
    async def on_submit(self, it: discord.Interaction):
        ...
        try:
            ...
            await it.followup.send(f"✅ **{self.title_input.value}** を {self.date_input.value} に登録しました！", ephemeral=True)
        except: await it.followup.send("❌ 形式エラー。日付や時間を確認してください。", ephemeral=True)
...
class ReminderMenuView(ui.View):
    ...
    async def list_events(self, it: discord.Interaction, button: ui.Button):
        ...
        for d, evs in sorted(grouped.items()):
            ...
            # --- 日付ごとの独立したEmbedカードを作成 ---
            # 今日の予定は「緑」、それ以外は「青」にする
            color = 0x2ecc71 if d == today_str else 0x3498db
            emb = discord.Embed(
                title=f"📅 {dt.strftime('%m/%d')} ({WEEKDAYS[dt.weekday()]})  {w}",
                color=color
            )
            ...
            for e in evs:
                ...
                time_icon = "⏰" if ":" in st else "☀️"
                time_str = f"`{st[11:16]}`" if ":" in st else "`終日`"
                lines.append(f"{time_icon} {time_str} **{e['summary']}**\n└ ID: `{e['id']}`")
            
            emb.description = "\n".join(lines)
            embeds.append(emb)

        # Discordは1メッセージにつき10個までEmbedを載せられる
    ...
    async def delete_event(self, it: discord.Interaction, button: ui.Button):
        ...
        class DelModal(ui.Modal, title="予定の削除"):
            ev_id = ui.TextInput(label="イベントIDを入力 (コピーして貼り付け)")
            async def on_submit(self, sit: discord.Interaction):
                try:
                    ...
                    await sit.response.send_message("🗑️ 削除しました。", ephemeral=True)
                except: await sit.response.send_message("❌ 失敗。IDを確認してください。", ephemeral=True)
...
def register_reminder_commands(bot, data_manager):
    ...
    class Reminder(app_commands.Group):
        ...
        async def menu(self, it: discord.Interaction):
            ...
            embed = discord.Embed(
                title="🗓️ カレンダー操作パネル", 
                description=f"今日は **{now.strftime('%m/%d')}({WEEKDAYS[now.weekday()]})** です。\n「25時」入力や遠い未来の予定も管理できます。", 
                color=0x4285F4
            )
        ...
        async def on(self, it: discord.Interaction, calendar_id: str):
            ...
            await it.response.send_message("✅ 設定完了！朝6時と10分前に通知します。")
                            ...
                            for e in gcal.get_events(cid, days=1):
                                ...
                                if st and e['id'] not in reminded_ids:
                                    ...
                                    if 0 < (dt - now).total_seconds() <= 600:
                                        await ch.send(content="@everyone", embed=discord.Embed(title="🕒 10分前リマインド", description=f"**{e['summary']}** が始まります！", color=0xe74c3c))
                                        reminded_ids.add(e['id'])
                        except: pass
            if len(reminded_ids) > 100: reminded_ids.clear()
            await asyncio.sleep(30)

    if not hasattr(bot, "_rem_loop_final"):
        bot._rem_loop_final = True
